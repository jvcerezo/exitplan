...
export default function ThirteenthMonthPage() {
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      <div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Estimate your 13th month and understand the ₱90,000 tax exemption.</p>
          </div>
        </div>
      </div>

      <Card className="rounded-2xl border border-border/60">
        <CardHeader className="pb-3"><CardTitle className="text-sm font-semibold">Calculator</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              <Label className="text-xs">Basic Monthly Salary (₱)</Label>
              <Input type="number" min="0" value={basicSalary} onChange={(e) => setBasicSalary(e.target.value)} placeholder="25000" className="h-9" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Months Worked This Year</Label>
              <Input type="number" min="0" max="12" value={monthsWorked} onChange={(e) => setMonthsWorked(e.target.value)} placeholder="12" className="h-9" />
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between py-2 px-3 rounded-xl bg-green-500/10">
              ...
              <div className="text-right">
                ...
                {result.taxExemptPortion >= 90000 && <Badge variant="secondary" className="ml-2 text-[9px]">Capped</Badge>}
              </div>
            </div>
            {result.taxable > 0 && (
      ...
      <Card className="rounded-2xl border border-border/60">
        <CardHeader className="pb-3"><CardTitle className="text-sm font-semibold">Things You Should Know</CardTitle></CardHeader>
      ...
      <Card className="rounded-2xl border border-border/60 bg-muted/20">
        <CardHeader className="pb-3"><CardTitle className="text-sm font-semibold">What Should You Do With It?</CardTitle></CardHeader>
        <CardContent className="space-y-3">
          {[
            ...
            <div key={item.label} className="flex items-start gap-3">
              <span className="shrink-0 text-[11px] font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-full">{item.pct}</span>
              <div>
