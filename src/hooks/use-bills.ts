...
export function useAddBill() {
  ...
  return useMutation({
    mutationFn: async (bill: BillInsert) => {
      ...
      const { data, error } = await supabase
        .from("bills").insert({ ...bill, user_id: user.id }).select().single();
      if (error) throw new Error(error.message);
      return data;
    },
    onSuccess: () => {
    ...
    onError: (e) => toast.error("Failed to add bill", { description: e.message }),
  });
}
...
export function useUpdateBill() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, ...updates }: Partial<BillInsert> & { id: string; is_active?: boolean; last_paid_date?: string | null }) => {
      const supabase = createClient();
      const { data, error } = await supabase
        .from("bills").update(updates).eq("id", id).select().single();
      if (error) throw new Error(error.message);
      return data;
    },
    onSuccess: () => {
    ...
    onError: (e) => toast.error("Failed to update bill", { description: e.message }),
  });
}
...
export function useDeleteBill() {
  ...
  return useMutation({
    ...
    onError: (e) => toast.error("Failed to delete bill", { description: e.message }),
  });
}

/** Mark a bill as paid. If an account is linked (bill.account_id or override accountId),
 *  also creates an expense transaction so the account balance is updated automatically. */
export function useMarkBillPaid() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ bill, accountId }: { bill: import("@/lib/types/database").Bill; accountId?: string }) => {
      const supabase = createClient();
      const today = new Date().toISOString().slice(0, 10);
      const effectiveAccountId = accountId ?? bill.account_id;

      // Always record the paid date on the bill
      ...
      // If linked to an account, create an expense transaction
    ...
    onError: (e) => toast.error("Failed to mark bill as paid", { description: e.message }),
  });
}
